    ...
    <section className="relative z-10 py-20 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          >
            <h2 className="text-5xl font-bold text-gray-900 mb-6">Gynger Pay</h2>
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="cursor-pointer">
              <Button className="bg-teal-600 hover:bg-teal-700 text-white font-semibold px-6 py-3 rounded-lg flex items-center gap-2 group cursor-pointer">
          ...
          >
            {/* Main Payment Card */}
            <motion.div
              className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 relative z-10 mb-4"
                ...
                <>
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    >
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                          selectedOption === "monthly" ? "bg-green-500" : "border-2 border-gray-300"
                        }`}
                      >
                        {selectedOption === "monthly" && <Check className="w-4 h-4 text-white" />}
                      </div>
                      <span className="text-lg text-gray-700">Pay monthly</span>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      onClick={() => setSelectedOption("net")}
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                          selectedOption === "net" ? "bg-green-500" : "border-2 border-gray-300"
                        }`}
                      >
                        {selectedOption === "net" && <Check className="w-4 h-4 text-white" />}
                      </div>
                      <span className="text-lg text-gray-700">Net terms</span>
                    </motion.div>
                  </div>

                  {/* Send Offer Button */}
                  <motion.button
                    className="w-full bg-gray-900 hover:bg-gray-800 text-white font-semibold py-4 rounded-lg transition-all cursor-pointer"
                    initial={{ y: 20, opacity: 0 }}
                    whileInView={{ y: 0, opacity: 1 }}
                    viewport={{ once: true }}
            ...
            {/* Mobile Card Preview */}
            <motion.div
              className="absolute -right-8 top-8 w-40 h-56 bg-gray-100 rounded-2xl shadow-lg border border-gray-200 p-4 transform rotate-12"
